...
NodoL* PostFijo(char* exp){
    ...
    while(exp[ind]){
        if(exp[ind]=='('){
            ...
            if(exp[ind]=='x'){
            ...
            } else if(IsFuncion(exp, ind)){
                copind=ind;
                if(IsFuncion(exp, ind)==1){
                    ind=ind+3;
                } else if(IsFuncion(exp, ind)==2){
                    ind=ind+4;
                } else if(IsFuncion(exp, ind)==3){
                    ind=ind+7;
                }
                ...
                while(!IsEmpty(pilop) && CompararOp(*(char*)Top(pilop), exp[ind])==1){
            ...
            expfn=CrearNodoL(expfn, &exp[ind]);
            if(!IsDigito(exp[ind+1]) && exp[ind+1]!='.') expfn=CrearNodoL(expfn, &coma1);
        }
        ind++;
    }
    while(pilop){
    ...
    while(exp[inda]!='\0'){
        if(exp[inda]!=','){
            ...
            if(IsDigito(exp[inda]) || exp[inda]=='x'){
                out->exp=CrearSubcadena(exp, &inda);
                out->op='N';
            } else if(IsFuncion(exp, inda)){
                el1=(Expifj*)Pop(&p1);//Argumento de la funcion
                tamt=TamanoCadena(el1->exp, 0)+3;
                if(IsFuncion(exp, inda)==1){
                    tamt=tamt+2;
                ...
                if((CompararOp(el1->op, exp[inda])==1 && CompararOp(el2->op, exp[inda])==-1) || (exp[inda]=='-' && (el2->op=='+' || el2->op=='-')) || (exp[inda]=='/' && el1->op=='N' && el2->op!='N')){
                    ...
                    out->exp[TamanoCadena(el1->exp, 0)]=exp[inda];
                    out->exp[TamanoCadena(el1->exp, 0)+1]='(';
                    for(int i=TamanoCadena(el1->exp, 0)+2, j=0; el2->exp[j]!='\0'; i++, j++) out->exp[i]=el2->exp[j];
                    out->exp[tamt]=')';
                    out->exp[tamt+1]='\0';
                } else if((CompararOp(el1->op, exp[inda])==-1 && CompararOp(el2->op, exp[inda])==-1) || (exp[inda]=='/' && el1->op!='N' && el2->op!='N')){
                    ...
                    out->exp[TamanoCadena(el1->exp, 0)+1]=')';
                    out->exp[TamanoCadena(el1->exp, 0)+2]=exp[inda];
